...
#### Overview: End-to-End Policy Lifecycle

```mermaid
graph TB
    Start([Policy Need Identified]) --> S1[Stage 1: Initiation & Authorization]
    S1 --> S2[Stage 2: Authoring & Collaboration]
    S2 --> S3[Stage 3: Review & Approval]
    S3 --> Decision{Approved?}
    Decision -->|Rejected| S2
    Decision -->|Approved| S4[Stage 4: Publication & Distribution]
    S4 --> S5[Stage 5: Acknowledgment & Attestation]
    S5 --> S6[Stage 6: Compliance Monitoring]
    S6 --> S7[Stage 7: Review & Revision]
    S7 --> ReviewDecision{Review Outcome}
    ReviewDecision -->|Reaffirm| S6
    ReviewDecision -->|Revise| S2
    ReviewDecision -->|Retire| S8[Stage 8: Archival & Retirement]
    S8 --> End([Policy Retired])
...
stateDiagram-v2
    ...
    InReview --> Draft: Rejected (Needs Revision)
    InReview --> Approved: Approval Granted
    Approved --> Active: Published
    Active --> UnderRevision: Change Initiated
    UnderRevision --> Draft: Draft New Version
    Active --> Archived: New Version Published
    Archived --> Obsolete: Policy Retired
    Active --> Obsolete: Policy Retired
    Obsolete --> [*]
...
sequenceDiagram
    ...
    participant Exco as Executive/Board
    participant System as Policy System

    Author->>System: Submit policy for review
    System->>System: Change state: Draft → In Review
    System->>Manager: Notification: Review required

    Manager->>System: Review & provide feedback
    alt Rejected by Manager
        System->>Author: Return with feedback
        Author->>System: Revise and resubmit
    else Approved by Manager
        System->>Owner: Route to Policy Owner
        Owner->>System: Review & approve

        alt Rejected by Owner
            System->>Author: Return with feedback
            Author->>System: Revise and resubmit
        else Approved by Owner
            par Parallel Review
                System->>Legal: Route for legal review
                System->>Exco: Route for executive review
            end

            Legal->>System: Approve
            Exco->>System: Approve

            System->>System: All approvals received
            System->>System: Change state: In Review → Approved
            System->>Author: Notification: Fully approved
            System->>System: Generate approval certificate
        end
    end
```
